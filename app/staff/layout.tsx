...
const navItems = [
  { href: '/staff', label: 'Operations Feed', icon: '⚡' },
  { href: '/staff/transport', label: 'Transport Fleet', icon: '🚌' },
  { href: '/staff/hostel', label: 'Hostel Matrix', icon: '🏨' },
];
      ...
      >
        ...
        <div className="h-16 flex items-center px-5 border-b border-white/[0.06]">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <span className="badge badge-purple px-1">OPS</span>
          </Link>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 pb-4 space-y-1">
          <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-widest px-3 py-2 mt-2">Core Tools</p>
          {navItems.map((item) => (
             <Link
                key={item.href}
                href={item.href}
